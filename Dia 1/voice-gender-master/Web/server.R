  ...
  if (grepl('vocaroo', tolower(url))) {
    ...
    if (grepl('mp3url', tolower(json))) {
      ...
      if (file.exists(wavFilePath)) {
        ...
        content <- paste0('<div class="shiny-output-error-validation">Error convirtiendo mp3 a wav.<br>Intenta convertirlo manualmente con <a href="http://media.io" target="_blank">media.io</a>.<br>Puedes descargar el mp3: <a href="', mp3, '">aquí.</a>.</div>')
        graph1 <- NULL
        graph2 <- NULL
...
process <- function(path) {
  ...
  tryCatch({
    ...
    incProgress(0.8, message = 'Creando gráfico 2/2 ..')
